  ...
  >
    ...
    <VCard class="refer-and-earn-dialog">
      <VCardText class="pa-5 pa-sm-16">
        <h3 class="text-h3 text-center mb-2">Add payment methods</h3>
        <p class="text-sm-body-1 text-center text-disabled">Supported payment methods</p>

        <div v-for="(item, index) in paymentMethodsData" :key="index">
          <div
            class="d-flex justify-space-between align-center py-4 text-high-emphasis font-weight-medium gap-x-4"
          >
            <div class="d-flex align-center">
              <VImg :src="item.img.value" height="30" width="50" class="me-3" />
              <div>{{ item.title }}</div>
            </div>
            <div class="d-none d-sm-block">{{ item.type }}</div>
          </div>
          <VDivider />
        </div>
      </VCardText>
