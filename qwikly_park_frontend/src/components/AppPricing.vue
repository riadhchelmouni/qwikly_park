...
<template>
  ...
  <div class="text-center">
    <h2 class="text-h2 pricing-title mb-2">{{ props.title ? props.title : 'Pricing Plans' }}</h2>
  ...
  <div class="d-flex align-center justify-center mx-auto my-10">
    ...
    <div class="position-relative">
      ...
      <div class="save-upto-chip position-absolute align-center d-none d-md-flex gap-1">
        <VIcon icon="tabler-corner-left-down" class="flip-in-rtl mt-2" />
    ...
    <VCol v-for="plan in pricingPlans" :key="plan.logo" v-bind="props" cols="12">
      ...
      <VCard flat border :class="plan.isPopular ? 'border-primary border-opacity-100' : ''">
        ...
        <VCardText>
          ...
          <!-- 👉 Plan name -->
          <h3 class="text-h3 mb-1 text-center">{{ plan.name }}</h3>
          <p class="mb-0 text-center">{{ plan.tagLine }}</p>
          ...
          <div class="position-relative">
            <div class="d-flex justify-center align-center py-8">
              ...
              >{{ annualMonthlyPlanPriceToggler ? Math.floor(Number(plan.yearlyPrice) / 12) : plan.monthlyPrice }}</h1>
            ...
            >{{ plan.yearlyPrice === 0 ? 'free' : `USD ${plan.yearlyPrice}/Year` }}</span>
          </div>
          ...
          >{{ plan.yearlyPrice === 0 ? 'Your Current Plan' : 'Upgrade' }}</VBtn>
        </VCardText>
      </VCard>
    </VCol>
  </VRow>
